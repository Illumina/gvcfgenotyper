...
{
    _variant_buffer.flush_buffer(chrom,pos);
...
int GVCFReader::read_lines(int num_lines)
{
	...
	int dp,num_dp=1;//depth
	if(bcf_get_format_int32(_bcf_header, _bcf_record, "DP", &dp , &num_dp)==1)//if DP is present, this is either a snp or a homref block and we want to store it in depth buffer;
	{
	    int num_format_values=1;
	    int end=_bcf_record->pos+1;
	    int gq,dpf;
	    bcf_get_format_int32(_bcf_header, _bcf_record, "END", &end , &num_format_values);
	    //if it is a SNP use GQ else use GQX (this is a illumina GVCF quirk)
	    if(_bcf_record->n_allele>1)
	    {
		bcf_get_format_int32(_bcf_header, _bcf_record, "GQ", &gq , &num_format_values)	;	
	    }
	    else
	    {
		bcf_get_format_int32(_bcf_header, _bcf_record, "GQX", &gq , &num_format_values);
	    }
	    bcf_get_format_int32(_bcf_header, _bcf_record, "DPF", &dpf , &num_format_values);
	    DepthBlock *db = new DepthBuffer(_bcf_record->rid,_bcf_record->pos,end,dp,dpf,gq);
	    _depth_buffer.push_back(db);
	}
    }
...
{
    while(_buffer.size()>0 &&  _buffer.front()->_end <= pos && _buffer.front()->_rid <= chrom)
    {
	DepthBlock *db = _buffer.front();
	_buffer.pop();
	delete db;
    }    
}
